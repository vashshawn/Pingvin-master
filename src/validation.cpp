...
#if defined(NDEBUG)
# error "Pingvincoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Pingvincoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetPingvincoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Pingvincoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("pingvincoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Pingvincoin: BIP30 has been active since inception
    ...
    // Pingvincoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetPingvincoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Pingvincoin: Disallow AuxPow blocks before it is activated.
    ...
    // Pingvincoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Pingvincoin: We probably want to disable this
    int nLockTimeFlags = 0;
