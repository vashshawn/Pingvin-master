...
CFeeRate GetPingvinFeeRate(int priority)
{
    switch(priority)
    {
    case SUCH_EXPENSIVE:
...
const std::string GetPingvinPriorityLabel(int priority)
{
    switch(priority)
    {
    case SUCH_EXPENSIVE:
...
CAmount GetPingvinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    {
        LOCK(mempool.cs);
    ...
    nMinFee += GetPingvinDustFee(tx.vout, nDustLimit);
...
CAmount GetPingvinDustFee(const std::vector<CTxOut> &vout, const CAmount dustLimit) {
    CAmount nFee = 0;
