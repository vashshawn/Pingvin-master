...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Pingvincoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no pingvincoin: URI
    if(!uri.isValid() || uri.scheme() != QString("pingvincoin"))
        return false;
...
{
    // Convert pingvincoin:// to pingvincoin:
    //
    //    Cannot handle this later, because pingvincoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("pingvincoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "pingvincoin:");
    }
...
{
    QString ret = QString("pingvincoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Pingvincoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Pingvincoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Pingvincoin (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Pingvincoin\n";
        else
            optionFile << strprintf("Name=Pingvincoin (%s)\n", chain);
