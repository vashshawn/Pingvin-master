...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Pingvin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no pingvin: URI
    if(!uri.isValid() || uri.scheme() != QString("pingvin"))
        return false;
...
{
    // Convert pingvin:// to pingvin:
    //
    //    Cannot handle this later, because pingvin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("pingvin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "pingvin:");
    }
...
{
    QString ret = QString("pingvin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Pingvin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Pingvin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Pingvin (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Pingvin\n";
        else
            optionFile << strprintf("Name=Pingvin (%s)\n", chain);
