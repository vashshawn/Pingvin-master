# Installing Pingvin Core

### Pre-compiled binaries

The easiest way to install the latest version of the Pingvin Core software is
by to download the latest precompiled binaries for your platform from the
[release page](https://github.com/pingvin/pingvin/releases). Currently,
...
It is possible to build your own copy of Pingvin Core with the exact, tested,
dependencies, as used for the binary releases, by using the
[depends system](depends/description.md). Please refer to the
[depends README](depends/README.md) for instructions to build Pingvin using
these dependencies.

### Compiling using system-provided libraries

  The following are developer notes on how to build Pingvin on your native
  ...
  - [macOS Build Notes](doc/Building-Pingvin-1.14-for-Mac.md)
...
Run with the `-testnet` option to run with "play pingvins" on the test network, if you
...
Pingvin Core is a multithreaded application, and deadlocks or other multithreading bugs
