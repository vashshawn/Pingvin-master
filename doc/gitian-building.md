...
- [Building Pingvincoin Core](#building-pingvincoin-core)
...
Clone the git repositories for pingvincoin and Gitian.
...
in the pingvincoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Pingvincoin Core
----------------

To build Pingvincoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the pingvincoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pingvincoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/pingvincoin/pingvincoin
...
For example:
```bash
URL=https://github.com/laanwj/pingvincoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit pingvincoin=${COMMIT} --url pingvincoin=${URL} ../pingvincoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pingvincoin=${COMMIT} --url pingvincoin=${URL} ../pingvincoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pingvincoin=${COMMIT} --url pingvincoin=${URL} ../pingvincoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the pingvincoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../pingvincoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/pingvincoin
SIGPATH=/some/root/path/pingvincoin-detached-sigs

./bin/gbuild --url pingvincoin=${BTCPATH},signature=${SIGPATH} ../pingvincoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pingvincoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pingvincoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/pingvincoin-osx-build.assert
```
...
[pingvincoin/gitian.sigs](https://github.com/pingvincoin/gitian.sigs/) repository, or if that's not possible create a pull
