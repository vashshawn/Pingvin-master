...
Below are some notes on how to build Pingvin Core for Windows.
...
Note that for WSL v1 the Pingvin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/pingvin, AND not under, for example, /mnt/d/pingvin. 
...
way. This will install to `c:\workspace\pingvin`, for example:

    make install DESTDIR=/mnt/c/workspace/pingvin
