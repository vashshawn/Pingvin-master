...
3. All content of the data directory, except for the `pingvincoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `pingvincoin.conf`       | User-defined configuration settings for `pingvincoind` or `pingvincoin-qt`; can be specified by `-conf` option
`./`               | `pingvincoind.pid`        | Stores the process ID (PID) of `pingvincoind` or `pingvincoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `pingvincoind` or `pingvincoin-qt`
...
`pingvincoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
