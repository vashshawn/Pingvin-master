...
You must install required dependencies to build a basic Pingvin daemon, optional dependencies may vary according to your requirements.
...
Pingvin Core's released binaries are built using the "depends system", which
...
**Optional dependencies** :  
```bash
# Qt (required for pingvin-qt GUI)
...
According to installed dependencies, the following steps will compile `pingvind`, `pingvin-cli` and `pingvin-qt`.
...
#### Enable Qt GUI
Create `pingvin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Pingvin Core expect BerkeleyDB 5.3**.
...
#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Pingvin may be compiled in
...
#### Security
To help make your Pingvin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Pingvin should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Pingvin Core.
