# Unix Build of Pingvincoin Core
...
You must install required dependencies to build a basic Pingvincoin daemon, optional dependencies may vary according to your requirements.
...
Pingvincoin Core's released binaries are built using the "depends system", which
...
**Optional dependencies** :  
```bash
# Qt (required for pingvincoin-qt GUI)
...
According to installed dependencies, the following steps will compile `pingvincoind`, `pingvincoin-cli` and `pingvincoin-qt`.
...
#### Enable Qt GUI
Create `pingvincoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Pingvincoin Core expect BerkeleyDB 5.3**.
...
#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Pingvincoin may be compiled in
...
#### Security
To help make your Pingvincoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Pingvincoin should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Pingvincoin Core.
